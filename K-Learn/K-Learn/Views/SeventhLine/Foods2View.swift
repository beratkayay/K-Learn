...
struct Foods2View: View {
    ...
    let questions = QuestionsForFoods().questions
            ...
            if currentQuestionIndex < questions.count   {
                ZStack {
                    ...
                    VStack {
                        ...
                        VStack(spacing: 20) {
                            // İlk üç seçeneği gösteren HStack
                            HStack {
                                ForEach(questions[currentQuestionIndex].answerOptions.prefix(3), id: \.self) { option in
                                    ZStack {
                                        Text(option)
                                        RoundedRectangle(cornerRadius: 20)
                                            ...
                                            .opacity(0.3)
                                            .onTapGesture {
                                                // Seçilen ifadeyi seçilen ifadeler listesine ekle
                                                addToSelectedPhrases(option)
                                            }
                                    }
                                }
                            }
                            // Son üç seçeneği gösteren HStack
                            HStack {
                                ForEach(questions[currentQuestionIndex].answerOptions.suffix(3), id: \.self) { option in
                                    ZStack {
                                        Text(option)
                                        RoundedRectangle(cornerRadius: 20)
                                            .frame(width: 100, height: 50)
